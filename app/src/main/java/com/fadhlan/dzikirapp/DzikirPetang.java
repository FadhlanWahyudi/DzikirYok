...
public class DzikirPetang extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        idDzikirpetang = getIntent().getStringExtra("KEY");
        petang = findViewById(R.id.petang);
        arrayList = new ArrayList<>();
        setdatasore();
        adapterDzikir = new AdapterDzikir(arrayList);

        petang.setLayoutManager(new LinearLayoutManager(this));
        petang.setHasFixedSize(true);
        petang.setAdapter(adapterDzikir);

    }
    private void setdatasore(){
        int count = 0;
        for (String id : judulpetang) {
            arrayList.add(new ModelDzikir(id,subjudulpetang[count]));
            count++;
        }
    }
    }
